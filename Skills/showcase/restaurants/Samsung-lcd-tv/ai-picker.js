...
{"channel_number": <number>, "reason": "<one short sentence, in the user's language>"}`;
    ...
    function isEditable(el) {
      ...
      // Inside our own AI modal? Treat as editable so chips/buttons there
      // never trigger TV shortcuts either.
      if (el.closest && el.closest('#ai-modal-backdrop')) return true;
      return false;
    }
    function keyboardGuard(e) {
      // Always let modifier-laden shortcuts through (Ctrl-R, Cmd-V, etc.)
      if (e.ctrlKey || e.metaKey || e.altKey) return;
      if (kbLocked || isEditable(e.target)) {
        e.stopPropagation();

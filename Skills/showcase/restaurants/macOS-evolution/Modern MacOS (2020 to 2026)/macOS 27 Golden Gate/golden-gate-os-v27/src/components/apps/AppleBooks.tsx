...
The A17 Pro brought hardware ray tracing, while the A19 Pro (2025) integrates neural accelerators directly into the GPU, enabling unprecedented on-device intelligence as new local models comes out from Apple.`,
  },
  {
    ...
    era: 'macOS 11-Golden Gate (2020-2026)',
...
The pinnacle of the "Modern Face," macOS Golden Gate introduces the "Liquid Glass" aesthetic. With refractive interfaces, a dedicated Phone app, and deep Apple Intelligence integration, it represents the complete realization of the Sovereign Ecosystem.`,
  },
  {
    id: 8,
...
### Golden Gate Utility Evolution
In macOS Golden Gate, Terminal adopts the Liquid Glass look with 24-bit color support, while Activity Monitor gains granular insights into neural engine workloads, ensuring the system remains stable and efficient.`,
  },
  {
    id: 9,
...
From the early days of Project Builder to the sophisticated LLM-integrated playgrounds of Golden Gate, the Guardian ensures every line of code is backed by Sovereign intelligence.`,
  },
  {
    id: 10,
...
### Liquid Media Engine
In the Golden Gate era, these tools utilize the M5 Max's dedicated media engines to handle 8K spatial video and neural audio mastering with zero latency, preserving the artistic soul of the ecosystem.`,
  },
  {
    id: 11,
...
The evolution continued with SwiftUI, enabling declarative "Liquid Glass" interfaces that are inherently reactive. The Guardian oversees the language's growth into a silicon-native powerhouse.`,
  },
  {
    ...
    era: 'Setup Assistant (Tiger-Golden Gate)',
    description: `### The First Hello
Hamdiya Fatima welcomes every user. From the iconic "Welcome" videos of OS X Tiger and Snow Leopard to the neural-authenticated setup of Golden Gate, the Noble Greeter ensures a seamless transition.
...
### Legacy Bridges
Even in the Golden Gate era, the Founding Soul ensures that the principles of the original 128K Mac—simplicity and user-centricity—remain at the core of the Liquid Glass experience.`,
  },
  {
    id: 14,
    codename: 'Afifa Khan',
...
Afifa Khan represents the impenetrable layer. From the introduction of FileVault in Panther to the Neural Secure Enclave of Golden Gate, the Shield protects the Sovereign user's data.
...
### Neural Continuity
In Golden Gate, the Bridge uses neural snapshots to predict which device you'll use next, ensuring your windows and state are already there before you even pick up your iPhone.
...
The Athlete pushes the M5 Max to its limits, enabling real-time ray tracing and liquid-glass refraction that defines the visual hallmark of macOS Golden Gate.`,
  },
];
        ...
        <div className="space-y-1">
          {[
            ...
            <div
              ...
              onClick={() => {
                if (!item.disabled) setView(item.id as any);
              }}
            >
              <item.icon size={18} className="hugeicon-golden-gate" />
          ...
          {view === 'store' ? (
            ...
            >
              ...
              <div className="flex gap-12 bg-zinc-100/50 dark:bg-white/5 p-8 rounded-[40px] border border-zinc-200 dark:border-white/10 shadow-xl">
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-between border border-white/10 rounded-lg">
                    <div>
                      ...
                      <h2 className="text-2xl font-serif font-bold text-white leading-tight">
                        The History of Apple: The "Her" Ecosystem
                      </h2>
                    </div>
                ...
                <div className="flex flex-col justify-center font-sans">
                  <div className="px-3 py-1 bg-blue-500/10 text-blue-500 rounded-full text-[10px] font-black w-fit mb-4 uppercase tracking-widest">
                    New Release
                  </div>
                  ...
                  <p className="text-zinc-500 mb-8 max-w-sm leading-relaxed">
                    A definitive 16-chapter chronicle exploring the Guardians of the Codebase and the architectural
                    evolution of the Golden Gate era.
                  </p>
            ...
            >
              <div className="flex items-center justify-between w-full mb-12 font-sans text-zinc-400">
                <button onClick={() => setCurrentChapterIndex((prev) => Math.max(0, prev - 1))}>
                  <ArrowLeft01Icon size={20} />
                </button>
                <div className="text-[10px] font-bold uppercase tracking-widest">
                  Chapter {chapters[currentChapterIndex].id} of {chapters.length}
                </div>
                <button onClick={() => setCurrentChapterIndex((prev) => Math.min(chapters.length - 1, prev + 1))}>
                  <ArrowRight01Icon size={20} />
                </button>
              </div>

              <div className="w-full text-center mb-12">
                <h3 className="text-sm font-sans font-bold text-blue-500 uppercase tracking-[0.3em] mb-4">
                  {chapters[currentChapterIndex].era}
                </h3>
                <h2 className="text-5xl font-bold mb-2 tracking-tight leading-tight">
                  {chapters[currentChapterIndex].codename}
                </h2>
                ...
                {chapters[currentChapterIndex].description.split('\n\n').map((paragraph, pIdx) => {
                  if (paragraph.startsWith('###')) {
                    return (
                      <h3
                        key={pIdx}
                        className="text-2xl font-sans font-bold text-zinc-900 dark:text-white mt-12 mb-4 tracking-tight"
                      >
                  ...
                  return (
                    <p
                      ...
                      className={
                        pIdx === 0
                          ? 'first-letter:text-7xl first-letter:font-bold first-letter:mr-3 first-letter:float-left first-letter:mt-3 first-letter:text-zinc-900 dark:first-letter:text-white'
                          : ''
                      }
                    >
                      {paragraph}

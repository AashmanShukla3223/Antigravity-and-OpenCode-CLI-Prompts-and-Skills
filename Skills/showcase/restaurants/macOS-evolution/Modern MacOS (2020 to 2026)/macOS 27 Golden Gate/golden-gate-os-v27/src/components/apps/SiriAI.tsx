...
export const SiriAI: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>(() => [
    {
      ...
      role: 'assistant',
      content:
        "Hello! I'm Siri, powered by Apple Intelligence 2.0 with multi-provider AI. Configure your API keys in settings to get started.",
      timestamp: Date.now(),
    },
  ]);
  ...
  const engine = useMemo(
    () => new AIEngine({ launchApp, updateSystemState, setPowerMode }),
    [launchApp, updateSystemState, setPowerMode],
  );
  ...
  const providerBadge = (provider?: AIProvider) => {
    ...
    return <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-full ${colors[provider]}`}>{config.name}</span>;
  };
        ...
        <div className="flex-1 flex flex-col overflow-y-auto p-8">
          <div className="max-w-lg mx-auto w-full space-y-6">
            ...
            <p className="text-xs text-white/40">
              Configure AI providers to enable real intelligence. Your API keys are stored locally and never sent
              anywhere except the respective API.
            </p>
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${apiKeys[provider.key] ? 'bg-green-400' : 'bg-white/20'}`}
                      />
            ...
            <div className="h-14 flex items-center justify-between px-5 border-b border-white/10 bg-white/5 backdrop-blur-[40px]">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-white/5 border border-white/10">
                  ...
                  >
                    <div
                      className={`w-2.5 h-2.5 rounded-full bg-white absolute top-0.5 transition-transform ${webSearchEnabled ? 'translate-x-4' : 'translate-x-0.5'}`}
                    />
                  </button>
                </div>
                <button
              ...
              <div className="px-6 pb-3">
                <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20">
                  ...
                  <p className="text-[10px] text-amber-400/60 mb-3">
                    Configure at least one AI provider to enable real responses.
                  </p>
                  <button
                    onClick={() => setShowSettings(true)}

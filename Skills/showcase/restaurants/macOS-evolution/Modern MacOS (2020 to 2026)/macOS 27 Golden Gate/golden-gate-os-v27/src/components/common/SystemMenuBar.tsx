...
export const SystemMenuBar: React.FC<SystemMenuBarProps> = ({ mode, showUtilities, onTerminalOpen }) => {
  ...
  const ssids = ['Apple Store Wi-Fi', 'Home Network 5G', 'Golden-Gate-Guest-Neural'];
  ...
  return (
    <div className="fixed top-0 left-0 right-0 h-[30px] flex justify-between items-center px-4 text-sm text-white z-[200] bg-black/40 backdrop-blur-xl border-b border-white/5 select-none">
      <div className="flex items-center h-full gap-1">
        ...
        <div className="px-3 h-full flex items-center font-bold tracking-tight">{mode}</div>
        {showUtilities && (
      ...
      <div className="flex items-center h-full gap-1">
        ...
        <div className="relative h-full">
          ...
          >
            <img
              src={FileSystemResolver.getDeviceIcon('network-wireless')}
              alt="Wi-Fi"
          ...
          <AnimatePresence>
            {wifiMenuOpen && (
              <>
                ...
                >
                  <div className="px-3 py-1 text-[10px] font-black text-white/30 uppercase tracking-widest mb-1">
                    Select Network
                  </div>
                  {ssids.map((ssid) => (
                    <div
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/20 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-xs text-white/40 mb-6 text-center">
                Enter the administrator password for this network.
              </p>
              ...
              <div className="flex gap-3 w-full">
                <button
                  ...
                  className="flex-1 py-2.5 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={handleConnect}
                  className="flex-1 py-2.5 bg-white text-black rounded-xl text-xs font-bold transition-all"
                >
                  Join
                </button>
              </div>

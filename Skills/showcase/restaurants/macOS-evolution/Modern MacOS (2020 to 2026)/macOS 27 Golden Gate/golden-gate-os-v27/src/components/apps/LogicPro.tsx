...
export const LogicPro: React.FC = () => {
  ...
  const [trackWaveforms, setTrackWaveforms] = useState<Map<string, number[] | undefined>>(new Map());

  useEffect(() => {
    setTrackWaveforms(new Map(tracks.map(t => [t.id, t.audioUrl ? waveformCache.current.get(t.audioUrl) : undefined])));
  }, [tracks]);
  ...
  const handleDownload = useCallback(() => {
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      const sample =
        Math.sin(2 * Math.PI * freq * t) * 0.3 * masterVolume +
    ...
    <div className="flex flex-col h-full w-full bg-[#1a1a1a] text-white select-none">
      <div className="h-10 bg-[#2d2d2d] border-b border-[#3c3c3c] flex items-center px-4 gap-3 shrink-0">
        ...
        <ImportFileButton
          ...
          parentId="music"
          accept=".mp3,.wav"
          onImport={(file, dataUrl) => addTrackFromFile(file.name, dataUrl)}
        />
        <button
          onClick={() => {
        ...
        <span className="text-xs text-gray-400">
          Bar {currentBar + 1}/{TOTAL_BARS} • {barToTime(currentBar)}
        </span>
        <div className="w-px h-5 bg-[#3c3c3c]" />
        <button
          onClick={() => {
      ...
      <div className="flex-1 flex min-h-0">
        <div className="flex flex-col w-64 bg-[#252525] border-r border-[#333] shrink-0">
                    ...
                    {level > 0
                      ? Array.from({ length: 12 }).map((_, i) => (
                          <div
                            ...
                            style={{
                              height: `${Math.min(100, ((level * (i + 1)) / 12) * 100)}%`,
                              backgroundColor: i > 8 ? '#ef4444' : i > 5 ? '#f59e0b' : track.color,
                            }}
                          />
                        ))
                      : Array.from({ length: 12 }).map((_, i) => {
                          ...
                          return (
                            <div
                              ...
                              style={{
                                height: h,
                                backgroundColor: isAudible ? track.color + '44' : track.color + '22',
                              }}
                            />
                          );
                        })}
          ...
          <div className="p-3 border-t border-[#333]">
            <button
              onClick={addTrack}
              className="w-full py-1.5 rounded text-xs bg-[#3c3c3c] hover:bg-[#4a4a4a] transition"
            >
              + Add Track
            </button>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0" {...dropHandlers}>
          <div className="h-8 bg-[#2d2d2d] border-b border-[#333] flex items-center px-3 text-[10px] text-gray-500 gap-2 shrink-0">
            {Array.from({ length: TOTAL_BARS }).map((_, i) => (
              <span key={i} className={`w-12 shrink-0 ${i % 4 === 0 ? 'text-gray-300 font-medium' : ''}`}>
                {i + 1}
              </span>
            ))}
          </div>
          <div className="flex-1 overflow-auto">
            <div className="relative" style={{ width: TOTAL_BARS * 48, minHeight: '100%' }}>
              {tracks.map((track) => {
                const waveform = trackWaveforms.get(track.id);
                return (
                  <div
                    key={track.id}
                    className="h-12 border-b border-[#2a2a2a] flex relative"
                    style={{ backgroundColor: selectedTrack === track.id ? '#ffffff08' : 'transparent' }}
                  >
                    {Array.from({ length: TOTAL_BARS }).map((_, bar) => {
                      ...
                      return (
                        <div
                          key={bar}
                          className={`w-12 h-full shrink-0 border-r border-[#222] flex items-end justify-center pb-1 ${bar % 4 === 0 ? 'border-[#333]' : ''}`}
                        >
                          {peak > 0.01 && (
                            <div
                    ...
                    {track.audioUrl && !waveform && (
                      <div className="absolute inset-0 flex items-center justify-center text-[10px] text-gray-500">
                        <button
                          onClick={() =>
                            decodeWaveform(track.audioUrl!, TOTAL_BARS).then(() => setTracks((prev) => [...prev]))
                          }
      ...
      <div className="h-8 bg-[#2d2d2d] border-t border-[#3c3c3c] flex items-center px-4 text-xs text-gray-400 shrink-0">
        {selectedTrackData && (
          <span>
            Track: {selectedTrackData.name} • Vol: {Math.round(selectedTrackData.volume * 100)}% • Pan:{' '}
            {selectedTrackData.pan.toFixed(1)}
          </span>
        )}
        ...
        >
          ...
          >
            ...
            {exportDone && (
              <div className="text-center mb-4">
                ...
                <p className="text-xs text-gray-400 mb-3">
                  {exportPreset.label} • {exportPreset.sampleRate / 1000}kHz
                </p>
              </div>
            )}
            ...
            <div className="flex gap-2">
              {!isExporting && !exportDone && (
                <>
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-xl bg-orange-600 hover:bg-orange-500 text-sm font-medium transition"
                  >
                    Export
                  </button>
                  <button
              ...
              {isExporting && (
                <button disabled className="flex-1 py-2.5 rounded-xl bg-orange-600/50 text-sm cursor-not-allowed">
                  Exporting...
                </button>
              )}
              {exportDone && (

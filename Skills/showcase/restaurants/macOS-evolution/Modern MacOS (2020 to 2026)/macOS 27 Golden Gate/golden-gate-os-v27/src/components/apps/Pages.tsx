...
import { TextBoldIcon, TextItalicIcon, TextUnderlineIcon, TextAlignLeftIcon, TextAlignCenterIcon, TextAlignRightIcon, PlusSignIcon, Delete02Icon } from 'hugeicons-react';
...
export const Pages: React.FC = () => {
  ...
  const [documents, setDocuments] = useState<PageDocument[]>(() => {
    ...
    } catch { return [{ id: generateId(), name: 'Untitled', content: '', updatedAt: Date.now() }]; }
  });
  const [activeDocId, setActiveDocId] = useState<string>(documents[0]?.id || '');
  const [formatting, setFormatting] = useState({ bold: false, italic: false, underline: false, fontSize: '16', alignment: 'left' });
  ...
  const updateContent = useCallback(() => {
    ...
    setDocuments(prev => prev.map(d => d.id === activeDoc.id ? { ...d, content: html, updatedAt: Date.now() } : d));
  }, [activeDoc]);
    ...
    return (
      <div className="h-full w-full bg-zinc-900 text-white flex flex-col items-center justify-center gap-6">
        ...
        <div className="flex gap-3">
          <button onClick={() => { localStorage.setItem(STORAGE_KEY, 'true'); setPaid(true); }} className="px-6 py-2.5 bg-blue-500 hover:bg-blue-600 rounded-xl font-medium text-sm transition">
            Continue
          </button>
        </div>
      </div>
    ...
    <div className="h-full w-full bg-zinc-900 text-white flex flex-col overflow-hidden">
      <div className="flex items-center gap-1 px-3 py-2 border-b border-white/10 bg-zinc-800/50">
        <select
          ...
          onChange={(e) => { setFormatting(prev => ({ ...prev, fontSize: e.target.value })); execCmd('fontSize', e.target.value); }}
        ...
        <button onClick={() => { setFormatting(prev => ({ ...prev, bold: !prev.bold })); execCmd('bold'); }} className={`p-1.5 rounded-md transition ${formatting.bold ? 'bg-blue-500 text-white' : 'hover:bg-white/10'}`}><TextBoldIcon size={16} /></button>
        <button onClick={() => { setFormatting(prev => ({ ...prev, italic: !prev.italic })); execCmd('italic'); }} className={`p-1.5 rounded-md transition ${formatting.italic ? 'bg-blue-500 text-white' : 'hover:bg-white/10'}`}><TextItalicIcon size={16} /></button>
        <button onClick={() => { setFormatting(prev => ({ ...prev, underline: !prev.underline })); execCmd('underline'); }} className={`p-1.5 rounded-md transition ${formatting.underline ? 'bg-blue-500 text-white' : 'hover:bg-white/10'}`}><TextUnderlineIcon size={16} /></button>
        <div className="w-px h-5 bg-white/10 mx-1" />
        <button onClick={() => execCmd('justifyLeft')} className="p-1.5 rounded-md hover:bg-white/10"><TextAlignLeftIcon size={16} /></button>
        <button onClick={() => execCmd('justifyCenter')} className="p-1.5 rounded-md hover:bg-white/10"><TextAlignCenterIcon size={16} /></button>
        <button onClick={() => execCmd('justifyRight')} className="p-1.5 rounded-md hover:bg-white/10"><TextAlignRightIcon size={16} /></button>
      </div>

      <div className="flex flex-1 overflow-hidden">
        <div className="w-56 bg-zinc-800/30 border-r border-white/10 flex flex-col overflow-y-auto p-2">
          <button onClick={createDoc} className="flex items-center gap-2 p-2 hover:bg-white/10 rounded-lg mb-2 transition text-sm">
          ...
          {documents.map(doc => (
            ...
            >
              ...
              />
              <button onClick={(e) => { e.stopPropagation(); deleteDoc(doc.id); }} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-white/10 rounded transition"><Delete02Icon size={12} /></button>
            </div>
          ))}
        </div>

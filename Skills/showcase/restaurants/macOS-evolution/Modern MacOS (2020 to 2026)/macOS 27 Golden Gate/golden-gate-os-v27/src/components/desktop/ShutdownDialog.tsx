...
import { PowerSocket01Icon, Alert01Icon } from 'hugeicons-react';
import { useSystem } from '../../contexts/SystemContext';

export const ShutdownDialog: React.FC = () => {
  const { showShutdownDialog, setShowShutdownDialog, initiateShutdown, setBootState } = useSystem();
  ...
  }, [showShutdownDialog, handleShutdown]);
  ...
  if (!showShutdownDialog) return null;
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[200] bg-black/20 backdrop-blur-sm pointer-events-auto">
        ...
        >
          <div className="w-20 h-20 rounded-full bg-white/5 flex items-center justify-center mb-8 border border-white/10 shadow-inner">
            <PowerSocket01Icon size={40} className="text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.6)]" />
          </div>

          <h2 className="text-2xl font-black mb-3 tracking-tight">Shut Down Mac</h2>
          <p className="text-white/50 text-sm leading-relaxed mb-10 px-4">
            Are you sure you want to shut down your computer now? It will shut down automatically in{' '}

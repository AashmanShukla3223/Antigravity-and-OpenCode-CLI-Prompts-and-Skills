    ...
    <div className="flex h-full w-full bg-white dark:bg-[#0A0A0A] text-black dark:text-white overflow-hidden font-sans relative">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          ...
          <div className="px-2 mb-4 space-y-1">
            {favorites.map((f) => (
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full ${getContactColor(f.id)} flex items-center justify-center text-[10px] font-bold border border-white/10`}
                >
                  {getInitials(f.name)}
                </div>
                ...
                <StarIcon
                  size={12}
                  className={selectedId === f.id ? 'text-white/50 ml-auto' : 'text-zinc-300 dark:text-zinc-600 ml-auto'}
                />
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-12 bg-white dark:bg-black/20 backdrop-blur-3xl relative">
        <div className="max-w-md w-full flex flex-col items-center">
          ...
          {/* Dial Button */}
          <button onClick={handleDial} className="group relative flex flex-col items-center gap-4 mb-16">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400 group-hover:text-green-500 transition-colors">
              Call via iPhone
            </span>
          </button>
          ...
          <div className="w-full grid grid-cols-2 gap-4">
            <div
              ...
              className="p-4 bg-zinc-50 dark:bg-white/5 rounded-2xl border border-zinc-200 dark:border-white/10 cursor-pointer hover:bg-zinc-100 dark:hover:bg-white/10 transition-all"
            >
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.3em] text-zinc-300 dark:text-zinc-700">
          ...
          <span>Encrypted via Golden Gate Continuity Bridge</span>
        </div>
      </div>

...
const emailsData = [
  {
    ...
    subject: 'Golden Gate OS Project Zero: Unit 7 Era',
    date: '10:42 AM',
    unread: true,
    body: 'Team, we are officially moving into the "Unit 7" era of Apple computing. Golden Gate OS 27 must bridge the gap between legacy desktop metaphors and our futuristic "Frozen Prompt" architectures. I expect full commitment to the Silicon-Native Liquid Glass aesthetic.',
  },
  {
    id: 2,
    ...
    date: 'Yesterday',
    unread: false,
    body: 'We have finalized the logistics for the upcoming Golden Gate Digital HQ expansion. All SVPs should review the workspace allocation by Friday.',
  },
  {
    id: 3,
...
export const Mail: React.FC = () => {
  ...
  return (
    <div className="flex h-full w-full bg-white dark:bg-[#1C1C1E] text-gray-800 dark:text-gray-200">
      {/* Sidebar - Pane 1 */}
      <div
        className={`w-48 flex-shrink-0 border-r border-black/5 dark:border-white/10 flex flex-col z-10 transition-colors ${systemState.sidebarMaterial === 'clear' ? 'bg-white/10 dark:bg-black/10 backdrop-blur-[60px] saturate-[180%]' : 'bg-white/40 dark:bg-black/30 backdrop-blur-3xl'}`}
      >
        ...
        <div className="flex flex-col gap-1 px-2">
          <div
            ...
            className={`px-2 py-1.5 rounded-md flex items-center gap-2 cursor-pointer text-sm font-medium ${activeFolder === 'Inbox' ? 'bg-blue-500 text-white shadow-sm' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-700 dark:text-gray-300'}`}
          >
            <Mail01Icon size={16} className="hugeicon-golden-gate" /> Inbox
          </div>
          <div
            onClick={() => setActiveFolder('Sent')}
            className={`px-2 py-1.5 rounded-md flex items-center gap-2 cursor-pointer text-sm font-medium ${activeFolder === 'Sent' ? 'bg-blue-500 text-white shadow-sm' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-700 dark:text-gray-300'}`}
          >
            <SentIcon size={16} className="hugeicon-golden-gate" /> Sent
          </div>
          <div
            onClick={() => setActiveFolder('Drafts')}
            className={`px-2 py-1.5 rounded-md flex items-center gap-2 cursor-pointer text-sm font-medium ${activeFolder === 'Drafts' ? 'bg-blue-500 text-white shadow-sm' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-700 dark:text-gray-300'}`}
          >
            <FileEditIcon size={16} className="hugeicon-golden-gate" /> Drafts
          </div>
          <div
            onClick={() => setActiveFolder('Restaurants')}
            className={`px-2 py-1.5 rounded-md flex items-center gap-2 cursor-pointer text-sm font-medium ${activeFolder === 'Restaurants' ? 'bg-blue-500 text-white shadow-sm' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-700 dark:text-gray-300'}`}
          >
            <StarIcon size={16} className="hugeicon-golden-gate" /> Restaurants
          </div>
        </div>
      </div>
      ...
      <div className="w-72 flex-shrink-0 border-r border-black/5 dark:border-white/10 flex flex-col bg-gray-50 dark:bg-[#1e1e1e]">
        <div className="h-14 border-b border-black/5 dark:border-white/10 flex items-center px-3">
          <div className="relative w-full">
            <Search01Icon
              size={14}
              className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 hugeicon-golden-gate"
            />
            <input
              type="text"
              placeholder="Search"
          ...
          {filteredEmails.map((email) => (
            ...
            >
              <div className="flex justify-between items-baseline">
                ...
                <span className={`text-xs ${selectedEmail?.id === email.id ? 'text-white/80' : 'text-gray-500'}`}>
                  {email.date}
                </span>
              </div>
              <div className="flex items-center gap-1.5">
                {email.unread && selectedEmail?.id !== email.id && (
                  <div className="w-2 h-2 rounded-full bg-blue-500 shrink-0" />
                )}
                <span
                  className={`text-xs font-medium truncate ${selectedEmail?.id === email.id ? 'text-white' : 'text-gray-700 dark:text-gray-300'}`}
                >
                  {email.subject}
                </span>
              </div>
              <p
                className={`text-xs line-clamp-2 ${selectedEmail?.id === email.id ? 'text-white/80' : 'text-gray-500'}`}
              >
                {email.body}
              </p>
            </div>
        ...
        <div className="h-14 border-b border-black/5 dark:border-white/10 flex items-center justify-between px-6 bg-white/50 dark:bg-black/20 backdrop-blur-md">
          <div className="flex gap-2">
            <button className="p-1.5 rounded hover:bg-black/5 dark:hover:bg-white/10 text-gray-600 dark:text-gray-300 transition-colors">
              <ArchiveIcon size={16} className="hugeicon-golden-gate" />
            </button>
            <button className="p-1.5 rounded hover:bg-black/5 dark:hover:bg-white/10 text-gray-600 dark:text-gray-300 transition-colors">
              <Edit01Icon size={16} className="hugeicon-golden-gate" />
            </button>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-8 font-sans">
          {selectedEmail ? (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
              ...
              <div className="text-sm leading-relaxed text-gray-800 dark:text-gray-200">{selectedEmail.body}</div>
            </motion.div>
          ) : (
            <div className="flex h-full items-center justify-center text-gray-400 text-sm">No Message Selected</div>
          )}
        </div>
      </div>
    </div>

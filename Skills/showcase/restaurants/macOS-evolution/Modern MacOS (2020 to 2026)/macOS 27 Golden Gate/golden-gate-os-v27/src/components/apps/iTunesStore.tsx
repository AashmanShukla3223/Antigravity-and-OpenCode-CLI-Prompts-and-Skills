...
import { ShoppingBag, CheckCircle2, Music, ShoppingBasket } from 'lucide-react';
  ...
  const handlePaymentSuccess = () => {
    if (selectedItem) {
      ...
      if (selectedItem.id === 'music_pass') {
        localStorage.setItem('golden_gate_music_unlocked', 'true');
    ...
    <div className="flex flex-col h-full bg-zinc-50 dark:bg-[#1e1e1e] text-zinc-900 dark:text-white p-8 overflow-y-auto scrollbar-hide">
      <div className="flex items-center gap-4 mb-12">
        ...
        <div>
          ...
          <p className="opacity-40 text-sm font-bold uppercase tracking-widest">Golden Gate Edition</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        {items.map((item) => (
          <div
            key={item.id}
            className="bg-zinc-200/50 dark:bg-white/5 border border-zinc-300 dark:border-white/10 rounded-3xl p-6 flex flex-col justify-between hover:bg-zinc-200 dark:hover:bg-white/10 transition-all group"
          >
            <div>
      ...
      <div className="pt-12 border-t border-zinc-200 dark:border-white/5">
        <h2 className="text-sm font-black uppercase tracking-widest opacity-20 mb-8 text-center">
        ...
        <div className="flex gap-6 overflow-x-auto pb-8 scrollbar-hide">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="min-w-[160px] aspect-square bg-zinc-200/50 dark:bg-white/5 rounded-3xl border border-zinc-300 dark:border-white/5 flex flex-col items-center justify-center gap-4 group cursor-pointer hover:scale-105 transition-transform"
            >

...
const QUESTION_WORDS = new Set(['what', 'who', 'where', 'when', 'why', 'how', 'is', 'are', 'can', 'could', 'would', 'should', 'do', 'does', 'did', 'tell', 'explain', 'define', 'meaning', 'search', 'find', 'show', 'list']);
...
export const Spotlight: React.FC = () => {
  ...
  const { results, isAsking } = useMemo(() => {
    if (!q) return { results: [] as SearchResult[], isAsking: false };
    ...
    if (upper === 'SM') { launchApp('messages'); setShowSpotlight(false); return { results: [] as SearchResult[], isAsking: false }; }
    if (upper === 'REMINDER') { launchApp('reminders'); setShowSpotlight(false); return { results: [] as SearchResult[], isAsking: false }; }
    if (upper === 'STICKY') { launchApp('stickies'); setShowSpotlight(false); return { results: [] as SearchResult[], isAsking: false }; }

    const matched = APP_RESULTS.filter(r => r.name.toLowerCase().includes(query.toLowerCase()));
    if (matched.length > 0) return { results: matched, isAsking: false };

    const isQuery = isQuestion(q.toLowerCase());
    return {
      results: [{
        id: 'ai',
  ...
  }, [q, upper, query, launchApp, setShowSpotlight]);

  const placeholder = useMemo(() => {
    if (!q) return 'Search or Ask';
    return isQuestion(q.toLowerCase()) ? 'Ask Siri' : 'Search or Ask';
  }, [q]);
  ...
  const onKey = (e: React.KeyboardEvent) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); if (results.length) setIndex(i => (i + 1) % results.length); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); if (results.length) setIndex(i => (i - 1 + results.length) % results.length); }
    else if (e.key === 'Enter') { e.preventDefault(); handleAction(results[index]); }
    ...
    <AnimatePresence>
      {showSpotlight && (
        ...
        >
          ...
          />
          <div
            className="absolute top-[20%] left-1/2 -translate-x-1/2 w-full max-w-2xl bg-black/40 backdrop-blur-[var(--glass-blur)] saturate-[190%] border border-white/20 rounded-[28px] shadow-[0_40px_100px_rgba(0,0,0,0.6)] overflow-hidden"
          >
        ...
        {results.length > 0 && (
          <div className="border-t border-white/10 p-2 max-h-[400px] overflow-y-auto scrollbar-hide">
            ...
            {results.map((result, i) => (
              ...
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${i === index ? 'bg-white/20' : 'bg-white/5'}`}>
                  <result.icon size={20} className="text-white" />
                </div>
                <div className="flex-1">
                  <div className="text-sm font-bold text-white tracking-tight">{result.name}</div>
                  <div className={`text-[10px] uppercase font-black tracking-widest ${i === index ? 'text-white/70' : 'text-white/30'}`}>
                    {result.type === 'ai' && isAsking ? 'Ask Siri' : result.type}
                  </div>
                </div>
                {i === index && (
                  <span className="text-xs text-white/50 font-medium whitespace-nowrap">⏎ {result.type === 'ai' ? 'Ask' : 'Open'}</span>
                )}
              </div>
            ))}
          </div>

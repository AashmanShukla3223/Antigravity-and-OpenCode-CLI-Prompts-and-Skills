  ...
  return (
    <motion.main
      className={`fixed inset-0 z-50 transition-colors duration-1000 ${
        systemState.appearance === 'dark'
          ? 'bg-black text-white'
          : systemState.appearance === 'light'
      ...
      {systemState.notchMode === 'static' && (
        <div className="fixed top-0 left-1/2 -translate-x-1/2 z-50">
          <div className="flex items-center justify-center gap-1.5 w-[140px] h-[30px] bg-black rounded-b-[18px] px-4">
            {battery.isCharging && <BatteryCharging01Icon size={11} className="text-green-400" />}
            {!battery.isCharging && battery.level < 0.2 && <BatteryLowIcon size={11} className="text-red-400" />}
          </div>
        </div>
      )}
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            ...
            >
              <div className="w-48 h-48 rounded-full border border-white/20 bg-white/5 backdrop-blur-xl shadow-[0_0_50px_rgba(0,150,255,0.3)] animate-spin-slow mb-8 flex items-center justify-center overflow-hidden">
                <GlobalIcon
                  size={80}
                  className="text-blue-400 drop-shadow-[0_0_20px_rgba(0,150,255,0.5)] hugeicon-golden-gate"
                />
              </div>
            ...
            >
              ...
              <div className="flex gap-6">
                {[
                  ...
                  <div
                    key={acc.name}
                    className="w-32 h-32 rounded-2xl bg-white/5 border border-white/10 backdrop-blur flex flex-col items-center justify-center hover:bg-white/10 transition cursor-pointer group"
                  >
                    <acc.icon
                      size={32}
                      className="mb-2 text-white/70 group-hover:text-white transition-colors hugeicon-golden-gate"
                    />
                    <span>{acc.name}</span>
                  </div>
                ))}
            ...
            >
              ...
              <div className="flex gap-6">
                {['From a Mac', 'From Windows XP Restaurant', 'Not Now'].map((mig) => (
                  <div
                    key={mig}
                    className="w-40 h-48 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur flex flex-col items-center justify-center hover:bg-white/10 transition cursor-pointer transform hover:scale-105"
                  >
          ...
          {step === 7 && (
            ...
            >
              ...
              >
                <p className="mb-4">
                  Welcome to macOS Golden Gate. By using this software, you agree to the principles of the Silicon
                  Surge. Welcome to Apple Inc.
                </p>
                ...
                <p className="mb-4">
                  3. The Architect (@Aashmanshukla3223) reserves the right to update the UI at any time.
                </p>
          ...
          {step === 10 && (
            ...
            >
              ...
              <p className="mt-12 text-lg font-medium bg-clip-text text-transparent bg-gradient-to-r from-white/90 to-white/50">
                "Hey Siri, set up my Mac."
              </p>
            </motion.div>
          )}
            ...
            >
              ...
              <div className="flex gap-6">
                {(['Light', 'Dark', 'Auto'] as const).map((mode) => (
                  ...
                  >
                    <div
                      className={`w-16 h-16 rounded-full mb-4 ${mode === 'Light' ? 'bg-white' : mode === 'Dark' ? 'bg-black border border-white/20' : 'bg-gradient-to-r from-white to-black border border-white/20'}`}
                    />
                    <span>{mode}</span>
                  </div>
                ))}
          ...
          {step === 12 && (
            <motion.div
              ...
              exit="exit"
            >
              <h2 className="text-4xl font-semibold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-white to-white/50">
                Setting Up Your Mac...
              </h2>

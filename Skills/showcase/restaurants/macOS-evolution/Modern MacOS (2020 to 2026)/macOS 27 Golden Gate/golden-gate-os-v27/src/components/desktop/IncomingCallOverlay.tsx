...
export const IncomingCallOverlay: React.FC = () => {
  const { incomingCall, setIncomingCall, updateSystemState, addNotification } = useSystem();
  ...
  const handleDecline = () => {
    ...
    if (incomingCall) {
      const appId = incomingCall.type === 'facetime' ? 'facetime' : 'phone';
      const typeLabel = incomingCall.type === 'facetime' ? 'FaceTime' : 'Phone';
      if (accepted) {
        const mins = Math.floor(callDuration / 60);
        const secs = callDuration % 60;
        addNotification({
          appId,
          title: `${typeLabel} Call Ended`,
          message: `Call with ${incomingCall.contact.name} — Accepted by you (${mins}:${secs.toString().padStart(2, '0')})`,
        });
      } else {
        addNotification({
          appId,
          title: `Missed ${typeLabel} Call`,
          message: `${incomingCall.contact.name} — Declined by you`,
        });
      }
    }
    ...
    <AnimatePresence>
      {!accepted ? (
        <motion.div
          ...
          className="fixed top-12 right-6 z-[1000] w-[360px] bg-black/40 backdrop-blur-[var(--glass-blur)] saturate-[200%] border border-white/20 rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col pointer-events-auto"
        >
          {/* Notification Header */}
        ...
        >
          <div className="relative w-full h-full flex flex-col items-center justify-center">
            {/* Camera Feed with Golden Gate Edge-Light Filter */}
  ...
  <button
    ...
    className={`w-14 h-14 rounded-full flex items-center justify-center transition-all border ${
      active
        ...
        : 'bg-black/20 text-white border-white/20 backdrop-blur-xl hover:bg-black/40'
    }`}
  >
    <Icon size={24} />
  </button>

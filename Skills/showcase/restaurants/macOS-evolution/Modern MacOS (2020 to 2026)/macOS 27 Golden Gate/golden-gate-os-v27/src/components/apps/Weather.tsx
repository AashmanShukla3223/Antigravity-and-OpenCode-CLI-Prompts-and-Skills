...
import type { GoldenGateWeatherData } from '../../utils/WeatherEngine';
...
const getGlassmorphicIcon = (code: number, size = 48) => {
  ...
  const iconClass = 'hugeicon-golden-gate drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]';
...
export const Weather: React.FC = () => {
  ...
  const fetchWeather = useCallback(
    async (targetLoc: string) => {
      if (!apiKey) return;
      setLoading(true);
      setError(null);
      try {
        const data = await WeatherEngine.fetchGoldenGateWeather(targetLoc, apiKey);
        setWeather(data);
      } catch (err: any) {
  ...
  const handleSaveKey = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    localStorage.setItem('golden_gate_weather_api_key', key);
    setApiKey(key);
  ...
  return (
    <div
      className={`h-full w-full bg-gradient-to-b ${bgGradient} text-white flex flex-col font-sans overflow-hidden transition-all duration-1000`}
    >
      {/* Top Bar */}
      ...
      {showSettings ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 bg-black/40 backdrop-blur-xl z-10">
          ...
          <p className="text-xs text-white/60 mb-8 text-center max-w-xs">
            Enter your WeatherAPI.com Key to enable real-time Golden Gate Intelligence.
          </p>
            ...
            {weather ? (
              <>
                <div className="pt-6 pb-8 flex flex-col items-center text-center relative">
                  ...
                  <div className="relative mb-2">{getGlassmorphicIcon(weather.conditionCode, 80)}</div>

...
const AUTHORIZED_ASSETS = [
  { name: 'iPhone Mirroring.md', type: 'System Logic', path: 'macOS 27 Golden Gate/' },
        ...
        {stage === 'booting' && (
          ...
          >
            ...
            <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden border border-white/5">
              <motion.div className="h-full bg-white" initial={{ width: 0 }} animate={{ width: `${bootProgress}%` }} />
            </div>
                ...
                <div className="space-y-2 pb-20">
                  {AUTHORIZED_ASSETS.map((logic, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center border ${isMissing ? 'bg-red-500/10 border-red-500/20' : 'bg-gradient-to-br from-blue-500/20 to-indigo-500/20 border-white/5'}`}
                        >
                          {isMissing ? (
                            <Alert01Icon size={14} className="text-red-400" />
                          ) : (
                            <img
                              src={`${base}${FileSystemResolver.getMimeIcon('document')}`}
                              className="w-4 h-4"
                              loading="lazy"
                            />
                          )}
                        ...
                        <div className="flex-1 min-w-0">
                          <div
                            className={`text-[10px] font-bold truncate ${isMissing ? 'text-red-200' : 'text-white'}`}
                          >
                            {logic.name}
                          </div>
                          <div
                            className={`text-[7px] font-bold uppercase tracking-tighter ${isMissing ? 'text-red-400/80' : 'text-white/40'}`}
                          >
                            {logic.type} &bull; {logic.path}
                          </div>
                        </div>
                      </motion.div>
                    );
                  ...
                  <div className="mt-4 p-4 bg-blue-600 rounded-3xl shadow-xl relative overflow-hidden group">
                    <div className="relative z-10 flex flex-col gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-white/60 text-[8px] leading-tight">
                          Surgical link verified for {HARDCODED_LOCK_PATH}
                        </p>
                      </div>
                    </div>

...
export const Keynote: React.FC = () => {
  ...
  const updateElement = useCallback(
    (elId: string, updates: Partial<SlideElement>) => {
      setSlides((prev) =>
        prev.map((s, i) =>
          i === activeSlideIndex
            ? {
    ...
    return (
      <div className="h-full w-full bg-zinc-900 text-white flex flex-col items-center justify-center gap-6">
        ...
        <button
          ...
          className="px-6 py-2.5 bg-orange-500 hover:bg-orange-600 rounded-xl font-medium text-sm transition"
        >
          Continue
        </button>
      </div>
      ...
      <div className="h-full w-full bg-black flex flex-col" onClick={() => setPresentMode(false)}>
        <div className="absolute top-4 left-4 z-20">
          <button
            ...
            className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm transition flex items-center gap-2"
          >
        ...
        <div className="flex-1 flex items-center justify-center p-12">
          <div
            className="w-full h-full max-w-5xl max-h-[80vh] rounded-2xl border border-white/10 relative overflow-hidden"
            style={{ backgroundColor: activeSlide?.backgroundColor || '#1a1a2e' }}
          >
            {activeSlide?.elements.map((el) =>
              el.type === 'text' ? (
                <div
                  ...
                  style={{ left: el.x, top: el.y, width: el.width, color: el.color, fontSize: '24px' }}
                >
                  {el.content}
                </div>
              ) : el.type === 'rect' ? (
    ...
    <div className="h-full w-full bg-zinc-900 text-white flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/10 bg-zinc-800/50">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-1.5 bg-orange-500 hover:bg-orange-600 rounded-lg text-xs font-medium transition"
        >
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-36 bg-zinc-800/30 border-r border-white/10 flex flex-col overflow-y-auto p-2">
          <button
            onClick={addSlide}
            className="flex items-center justify-center p-2 hover:bg-white/10 rounded-lg mb-2 transition"
          >
            ...
            >
              <div className="absolute inset-0 flex items-center justify-center">
                {slide.elements.slice(0, 2).map((el) =>
                  el.type === 'text' ? (
                    <div key={el.id} className="text-[6px] font-bold truncate px-1" style={{ color: el.color }}>
                      {el.content}
                    </div>
                  ) : (
                    <div
                      ...
                      style={{ width: 12, height: 12, backgroundColor: el.color }}
                    />
                  ),
                )}
              </div>
              <div className="absolute top-1 right-1 flex gap-0.5 opacity-0 group-hover:opacity-100 transition">
                <button
                  ...
                  className="p-0.5 bg-black/50 rounded text-[8px] hover:bg-black/80"
                >
                  ▲
                </button>
                <button
                ...
                <button
                  ...
                  className="p-0.5 bg-red-500/50 rounded text-[8px] hover:bg-red-500"
                >
                  <Delete02Icon size={8} />
                </button>
              </div>
        ...
        <div className="flex-1 flex items-center justify-center bg-zinc-800/20 overflow-hidden">
          <div
            className="w-[90%] h-[85%] rounded-2xl border border-white/10 relative overflow-hidden shadow-2xl"
            style={{ backgroundColor: activeSlide?.backgroundColor || '#1a1a2e' }}
          >
            {activeSlide?.elements.map((el) => (
              <div
              ...
              >
                {el.type === 'text' ? (
                  ...
                  <div
                    ...
                    style={{ backgroundColor: el.color, borderColor: el.color }}
                  />
                ) : (
                  <div

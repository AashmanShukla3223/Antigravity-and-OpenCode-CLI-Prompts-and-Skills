          ...
          {filteredContacts.map((contact) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-full ${getContactColor(contact.id)} flex items-center justify-center text-white font-bold text-sm shadow-inner relative`}
                >
      ...
      <div className="flex-1 flex flex-col relative bg-black">
        <AnimatePresence>
          {isCalling && selectedContact ? (
            ...
            >
              <div
                className={`w-24 h-24 rounded-full ${getContactColor(selectedContact.id)} flex items-center justify-center text-white text-3xl font-black mb-6 shadow-2xl animate-pulse`}
              >
                {getInitials(selectedContact.name)}
              </div>
              <h2 className="text-3xl font-bold mb-2">{selectedContact.name}</h2>
              <p className="text-white/40 font-medium uppercase tracking-[0.3em] text-[10px] mb-12">
                FaceTime Video...
              </p>
              <button
          ...
          {!_error && !isVideoOff && (
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 max-w-[80%] bg-black/50 backdrop-blur-md rounded-2xl px-6 py-3 border border-white/10 flex items-center gap-4 z-30 shadow-2xl">
              ...
              <p className="text-sm font-medium text-white/90">
                "Hey, are you seeing the new liquid glass materials in Golden Gate? It looks incredible."
              </p>
            </div>
          )}
        </div>

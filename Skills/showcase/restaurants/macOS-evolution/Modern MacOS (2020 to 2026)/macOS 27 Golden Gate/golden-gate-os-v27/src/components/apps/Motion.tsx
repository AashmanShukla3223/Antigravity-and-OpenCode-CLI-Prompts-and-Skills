  ...
  const addElement = useCallback(
    (type: MotionElement['type']) => {
      ...
      const newElem: MotionElement = {
        ...
        opacity: 1,
        color: colors[type],
        content: type === 'text' ? 'Text' : type === 'shape' ? 'circle' : '',
        visible: true,
      };
    ...
    <div className="flex flex-col h-full w-full bg-[#1a1a1a] text-white select-none">
      <div className="h-11 bg-[#2d2d2d] border-b border-[#3c3c3c] flex items-center px-4 gap-3 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={addKeyframe}
            className="px-2 py-1 rounded text-xs bg-blue-600 hover:bg-blue-500 transition font-medium"
          >
            + Keyframe
          </button>
        </div>
        ...
        <span className="text-xs text-gray-400 ml-auto">
          {timeStr(currentFrame)} / {timeStr(totalFrames)}
        </span>
      </div>
            ...
            {elements
              ...
              .map((el) => {
                ...
                return (
                  ...
                  >
                    {el.type === 'text' && (
                      <div className="text-4xl font-bold whitespace-nowrap drop-shadow-lg" style={{ color: el.color }}>
                        {el.content}
                      </div>
                    )}
                    {el.type === 'shape' &&
              ...
              <>
                ...
                <div className="pt-2 border-t border-[#333]">
                  <label className="text-[10px] text-gray-500 uppercase tracking-wider block mb-1">
                    Keyframes ({elementKeyframes.length})
                  </label>

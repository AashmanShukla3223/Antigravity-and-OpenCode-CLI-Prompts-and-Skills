...
import { Calendar01Icon, Video01Icon, MusicNote01Icon, Sun01Icon, Grid02Icon, Cancel01Icon } from 'hugeicons-react';
...
const WIDGET_TEMPLATES = [
  ...
  { type: 'all-apps', name: 'All Apps', icon: Grid02Icon, color: 'bg-zinc-500' },
  {
    ...
    name: 'Devices',
    customIconUrl: FileSystemResolver.getDeviceIcon('phone-apple-iphone'),
    color: 'bg-zinc-800',
  },
] as const;
...
export const WidgetPicker: React.FC = () => {
  ...
  const addWidget = (type: Widget['type']) => {
    ...
    let nextY = 0;
    const sortedWidgetsAtX0 = systemState.widgets.filter((w) => w.x === 0).sort((a, b) => a.y - b.y);
          ...
          >
            ...
            <div className="flex-1 overflow-x-auto px-8 pb-8 flex items-center gap-6 scrollbar-hide">
              {WIDGET_TEMPLATES.map((tmpl) => (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-2xl ${tmpl.color} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform`}
                  >
                    {(() => {
